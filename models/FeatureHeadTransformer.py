...
class Model(nn.Module):
    """
        Long Time Series Forecasting via Feature Head Transformer
    """
    ...
    def __init__(self, configs):
        ...
        self.input_token_size = int((self.seq_len-self.win_len)/self.win_stride+1) + 1

        # build model
        self.featureHead = nn.Parameter(torch.randn(self.d_model))
        ...
        self.predict = nn.Linear(self.d_model, self.pred_len)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        inputTokens = torch.cat((self.featureHead.repeat(x_enc.size(0), self.enc_in, 1, 1), inputTokens), dim=2)
        ...
        # get featureHead
        featureHead = inputTokens.view(-1, self.enc_in, self.input_token_size, self.d_model)[:, :, 0, :]

        # mix channel if 1
        if self.channel_attn == 1:
            featureHead = self.channelBatchNorm(self.attn(featureHead, featureHead, featureHead)[0]) + featureHead

        y = self.predict(featureHead).permute(0,2,1) # b,s,c
